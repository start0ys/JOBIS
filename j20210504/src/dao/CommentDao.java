...
public class CommentDao {
	...
	public int insert(Comment comment) throws SQLException {
		...
		try {
			...
			pstmt.setInt(3, comment.getM_num());
			pstmt.setString(4, comment.getNickname());
			pstmt.setString(5, comment.getC_content());
	...
	public Comment select(int c_idx) throws SQLException {
		...
		try {
			...
			if(rs.next()){
				...
				comment.setM_num(rs.getInt("m_num"));
	...
	public int update(int c_idx, String c_content) throws SQLException {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int result = 0;
		String sql = "update comment1 set c_content=? where c_idx = ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
